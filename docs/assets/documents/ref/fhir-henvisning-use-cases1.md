# Fhir Henvisning Use Cases 1 - DK MEDCOM REFERRAL LM v0.1.0

* [**Table of Contents**](toc.md)
* **Fhir Henvisning Use Cases 1**
...
**Primær FHIR-ressource:** `ServiceRequest` **Triggerhændelse:** Klinisk beslutning om at henvise patient
...
**Primær FHIR-ressource:** `DiagnosticReport`, `Media`, `DocumentReference` **Triggerhændelse:** Behov for at understøtte klinisk beslutningsgrundlag
...
**Primær FHIR-ressource:** `ServiceRequest` (PUT / revision) **Triggerhændelse:** Fejl opdaget eller klinisk situation ændret efter afsendelse
...
**Primær FHIR-ressource:** `ServiceRequest` (status = revoked) **Triggerhændelse:** Patienten ønsker ikke forløbet, eller klinisk grundlag er bortfaldet
...
**Primær FHIR-ressource:** `Subscription`, `SubscriptionStatus`, `ServiceRequest` **Triggerhændelse:** Behov for overblik over egne afsendte henvisninger
...
**Primær FHIR-ressource:** `Task`, `SubscriptionNotification` **Triggerhændelse:** Visitatoren har truffet afgørelse om henvisningen
...
**Primær FHIR-ressource:** `Bundle`, `MessageHeader`, `ServiceRequest` **Triggerhændelse:** Indkommende henvisning i FHIR-endpoint
...
**Primær FHIR-ressource:** `Task` (triage-outcome, priority) **Triggerhændelse:** Ny eller opdateret henvisning klar til klinisk vurdering
...
**Primær FHIR-ressource:** `ServiceRequest` (active), `Appointment` (booked) **Triggerhændelse:** Positiv visitationsafgørelse
...
**Primær FHIR-ressource:** `Task` (declined + reason), `ServiceRequest` **Triggerhændelse:** Indikation ikke opfyldt, forkert visitationsspor, eller kapacitetsloft nået
...
**Primær FHIR-ressource:** `ServiceRequest` (replaces), `Task`, `Communication` **Triggerhændelse:** Forkert modtager eller bedre egnet tilbud identificeret
...
**Primær FHIR-ressource:** `ServiceRequest` (priority update), `SubscriptionNotification` **Triggerhændelse:** Ny information ændrer klinisk hastegrad
...
**Primær FHIR-ressource:** `CommunicationRequest` (fra visitator til henviser) **Triggerhændelse:** Utilstrækkeligt grundlag for visitationsafgørelse
...
**Primær FHIR-ressource:** `Communication` (reply, in-response-to) **Triggerhændelse:** Modtagelse af CommunicationRequest fra visitatoren
...
**Primær FHIR-ressource:** `Communication` (thread med in-response-to-kæde) **Triggerhændelse:** Faglig uklarhed der kræver mere end én udveksling
...
**Primær FHIR-ressource:** `SubscriptionNotification`, `ServiceRequest` **Triggerhændelse:** Ethvert statusskift på en aktiv henvisning hos visitatoren
...
**Primær FHIR-ressource:** `Communication`, `Task`, `ServiceRequest` (alternativ) **Triggerhændelse:** Afvisning kombineret med behov for at finde alternativ løsning for patienten
...
**Primær FHIR-ressource:** `Communication`, `ServiceRequest` (PUT/korrektion) **Triggerhændelse:** Fejl opdaget under modtagelse eller triage hos visitatoren
